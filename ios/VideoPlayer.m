...
static int const VideoPlayerUnset = -1;
...
#if __has_include(<react-native-video/VideoPlayerCache.h>)
  VideoPlayerCache * _videoCache;
#endif
#if TARGET_OS_IOS
...
#endif
#if __has_include(<react-native-video/VideoPlayerCache.h>)
    _videoCache = [VideoPlayerCache sharedInstance];
#endif
...
- (VideoPlayerViewController*)createPlayerViewController:(AVPlayer*)player
...
- (void)sendProgressUpdate
{
  ...
  [[NSNotificationCenter defaultCenter] postNotificationName:@"VideoPlayer_progress" object:nil userInfo:@{@"progress": [NSNumber numberWithDouble: currentTimeSecs / duration]}];
...
- (void)playerItemForSourceUsingCache:(NSString *)uri assetOptions:(NSDictionary *)options withCallback:(void(^)(AVPlayerItem *))handler {
  ...
  [_videoCache getItemForUri:uri withCallback:^(VideoPlayerCacheStatus videoCacheStatus, AVAsset * _Nullable cachedAsset) {
    switch (videoCacheStatus) {
      case VideoPlayerCacheStatusMissingFileExtension: {
      ...
      case VideoPlayerCacheStatusUnsupportedFileExtension: {
...
- (void) setSideloadedText {
  ...
  int selectedTrackIndex = VideoPlayerUnset;
  ...
  if (![type isEqualToString:@"disabled"] && selectedTrackIndex == VideoPlayerUnset) {
  ...
  for (int i = firstTextIndex; i < _player.currentItem.tracks.count; ++i) {
    BOOL isEnabled = NO;
    if (selectedTrackIndex != VideoPlayerUnset) {
...
- (BOOL)setLicenseResultError:(NSString *)error {
  if (_loadingRequest != nil) {
    NSError *licenseError = [NSError errorWithDomain: @"VideoPlayer"
                                                code: VideoPlayerErrorFromJSPart
                                            userInfo: @{
  ...
  if (_loadingRequest && error != nil) {
    ...
    if (self.onVideoError) {
      self.onVideoError(@{@"error": @{@"code": [NSNumber numberWithInteger: error.code],
                                      ...
                                      @"localizedFailureReason": [error localizedFailureReason] == nil ? @"" : [error localizedFailureReason],
                                      @"localizedRecoverySuggestion": [error localizedRecoverySuggestion] == nil ? @"" : [error localizedRecoverySuggestion],
                                      @"domain": _playerItem.error == nil ? @"VideoPlayer" : _playerItem.error.domain},
                          @"target": self.reactTag});
    }
  }
  return NO;
  ...
  if (self->_drm != nil) {
    ...
    if ([drmType isEqualToString:@"fairplay"]) {
      ...
      if (certificateStringUrl != nil) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
          ...
          if (certificateData != nil) {
            ...
            if (dataRequest != nil) {
                  ...
                  NSURLSessionDataTask *postDataTask = [session dataTaskWithRequest:request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
                    ...
                    if (error != nil) {
                      ...
                      if([httpResponse statusCode] != 200){
                        NSLog(@"Error getting license from %@, HTTP status code %li", url, (long)[httpResponse statusCode]);
                        NSError *licenseError = [NSError errorWithDomain: @"VideoPlayer"
                                                                    code: VideoPlayerErrorLicenseRequestNotOk
                                                                userInfo: @{
                        ...
                        NSError *licenseError = [NSError errorWithDomain: @"VideoPlayer"
                                                                    code: VideoPlayerErrorNoDataFromLicenseRequest
                                                                userInfo: @{
              ...
              NSError *licenseError = [NSError errorWithDomain: @"VideoPlayer"
                                                          code: VideoPlayerErrorNoDataRequest
                                                      userInfo: @{
                                                                  NSLocalizedDescriptionKey: @"Error obtaining DRM license.",
            ...
            NSError *licenseError = [NSError errorWithDomain: @"VideoPlayer"
                                                        code: VideoPlayerErrorNoCertificateData
                                                    userInfo: @{
                                                                NSLocalizedDescriptionKey: @"Error obtaining DRM license.",
        ...
        NSError *licenseError = [NSError errorWithDomain: @"VideoPlayer"
                                                    code: VideoPlayerErrorNoCertificateURL
                                                userInfo: @{
                                                            NSLocalizedDescriptionKey: @"Error obtaining DRM License.",
      ...
      NSError *licenseError = [NSError errorWithDomain: @"VideoPlayer"
                                                  code: VideoPlayerErrorNoFairplayDRM
                                              userInfo: @{
                                                          NSLocalizedDescriptionKey: @"Error obtaining DRM license.",
    ...
    NSError *licenseError = [NSError errorWithDomain: @"VideoPlayer"
                                                code: VideoPlayerErrorNoDRMData
                                            userInfo: @{
                                                        NSLocalizedDescriptionKey: @"Error obtaining DRM license.",
