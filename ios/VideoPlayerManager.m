...
- (UIView *)view
{
  return [[VideoPlayer alloc] initWithEventDispatcher:self.bridge.eventDispatcher];
}
...
{
    [self.bridge.uiManager prependUIBlock:^(__unused RCTUIManager *uiManager, NSDictionary<NSNumber *, VideoPlayer *> *viewRegistry) {
        VideoPlayer *view = viewRegistry[reactTag];
        if (![view isKindOfClass:[VideoPlayer class]]) {
            RCTLogError(@"Invalid view returned from registry, expecting VideoPlayer, got: %@", view);
        } else {
