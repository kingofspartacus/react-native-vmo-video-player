...
@implementation VideoPlayerCache
...
+ (VideoPlayerCache *)sharedInstance {
  static VideoPlayerCache *sharedInstance = nil;
...
- (NSString *)generateCacheKeyForUri:(NSString *)uri {
  ...
  if ([pathExtension isEqualToString:@""]) {
    ...
    NSError *error = [NSError errorWithDomain:@"VideoPlayerCache"
                                         code:VideoPlayerCacheStatusMissingFileExtension userInfo:userInfo];
    @throw error;
    ...
    NSError *error = [NSError errorWithDomain:@"VideoPlayerCache"
                                         code:VideoPlayerCacheStatusUnsupportedFileExtension userInfo:userInfo];
    @throw error;
  }
  return [[self generateHashForUrl:uri] stringByAppendingPathExtension:pathExtension];
}

- (void)getItemForUri:(NSString *)uri withCallback:(void(^)(VideoPlayerCacheStatus, AVAsset * _Nullable)) handler {
  @try {
    NSString *key = [self generateCacheKeyForUri:uri];
    AVURLAsset * temporaryAsset = [self getItemFromTemporaryStorage:key];
    if (temporaryAsset != nil) {
      handler(VideoPlayerCacheStatusAvailable, temporaryAsset);
      return;
    }
    ...
    [self.videoCache loadDataForKey:key withCallback:^(SPTPersistentCacheResponse * _Nonnull response) {
      if (response.record == nil || response.record.data == nil) {
        handler(VideoPlayerCacheStatusNotAvailable, nil);
        return;
      }
      [self saveDataToTemporaryStorage:response.record.data key:key];
      handler(VideoPlayerCacheStatusAvailable, [self getItemFromTemporaryStorage:key]);
    } onQueue:dispatch_get_main_queue()];
  } @catch (NSError * err) {
    switch (err.code) {
      case VideoPlayerCacheStatusMissingFileExtension:
        handler(VideoPlayerCacheStatusMissingFileExtension, nil);
        return;
      case VideoPlayerCacheStatusUnsupportedFileExtension:
        handler(VideoPlayerCacheStatusUnsupportedFileExtension, nil);
        return;
      default:
        @throw err;
    }
